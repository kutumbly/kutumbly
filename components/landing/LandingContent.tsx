...
export default function LandingContent({ forcedLang }: { forcedLang?: Language }) {
  const { lang, setLang } = useAppStore();
  const t = useTranslation(forcedLang || lang);

  // Sync store lang if forced (from URL param)
  useEffect(() => {
    if (forcedLang && forcedLang !== lang) {
      setLang(forcedLang);
    }
