...
export default function DiaryModule() {
  ...
  const [cEntryType, setCEntryType] = useState<'reflection' | 'activity' | 'milestone'>('reflection');
  const [cVisibility, setCVisibility] = useState<'normal' | 'vault'>('normal');
  ...
  const handleSaveEntry = () => {
    ...
    addEntry({
      ...
      is_locked: cVisibility === 'vault' ? 1 : 0
    });
    // Reset compose
    setCTitle(''); setCSubtitle(''); setCContent(''); setCTags('');
    setCMood('neutral'); setCWeather(''); setCLocation('');
    setCEntryType('reflection'); setCVisibility('normal');
  ...
  const getBreadcrumbs = () => {
    const b = [t('DIARY')];
    if (view === 'timeline' || view === 'reading') b.push(selectedMonth || t('sovereign_journal.timeline'));
    if (view === 'reading') b.push(activeEntry?.title || t('sovereign_journal.reflection'));
    return b;
  };
  ...
  const verifyPinAndUnlock = () => {
    if (authPin === currentPin && showPinPrompt) {
      ...
      alert(t('sovereign_journal.invalid_pin'));
      setAuthPin('');
    }
  };
  ...
  return (
    <ModuleShell
      title={
        view === 'overview' ? t('sovereign_journal.title') :
          view === 'timeline' ? `${selectedMonth} Archive` :
            t('sovereign_journal.title')
      }
      subtitle={view === 'overview' ? t('product.module.diary.desc') : undefined}
      onAdd={view === 'overview' && !showCompose ? () => setShowCompose(true) : undefined}
      addLabel={t('sovereign_journal.new_entry')}
                ...
                >
                  <div className="flex items-center justify-between border-b border-border-light pb-4">
                    <h3 className="text-[11px] font-black text-text-tertiary uppercase tracking-[0.3em]">
                      {t('sovereign_journal.compose_title')}
                    </h3>
                  ...
                  <div className="flex flex-col gap-5">
                    <input
                      autoFocus
                      placeholder={t('sovereign_journal.title_ph')}
                    ...
                    <textarea
                      placeholder={t('sovereign_journal.content_ph')}
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      <div className="flex flex-col gap-2">
                        <label className="text-[9px] font-black text-text-tertiary uppercase tracking-widest flex items-center gap-1.5"><Smile size={10} /> {t('sovereign_journal.mood_label')}</label>
                        <select value={cMood} onChange={e => setCMood(e.target.value)} className="bg-bg-secondary border border-border-light rounded-xl p-3 text-xs font-black uppercase text-text-primary focus:border-gold outline-none">
                          {MOODS.map(m => <option key={m} value={m}>{t(`sovereign_journal.mood_${m}`)}</option>)}
                        </select>
                      </div>
 
                      <div className="flex flex-col gap-2">
                        <label className="text-[9px] font-black text-text-tertiary uppercase tracking-widest flex items-center gap-1.5"><Tag size={10} /> {t('sovereign_journal.tags_label')}</label>
                        <input type="text" placeholder={t('sovereign_journal.tag_ph')} value={cTags} onChange={e => setCTags(e.target.value)} className="bg-bg-secondary border border-border-light rounded-xl p-3 text-xs font-bold text-text-primary focus:border-gold outline-none" />
                      </div>
 
                      <div className="flex flex-col gap-2">
                        <label className="text-[9px] font-black text-text-tertiary uppercase tracking-widest flex items-center gap-1.5"><MapPin size={10} /> {t('sovereign_journal.loc_label')}</label>
                        <input type="text" placeholder={t('sovereign_journal.loc_ph')} value={cLocation} onChange={e => setCLocation(e.target.value)} className="bg-bg-secondary border border-border-light rounded-xl p-3 text-xs font-bold text-text-primary focus:border-gold outline-none" />
                      </div>
 
                      <div className="flex flex-col gap-2">
                        <label className="text-[9px] font-black text-text-tertiary uppercase tracking-widest flex items-center gap-1.5"><Cloud size={10} /> {t('sovereign_journal.weather_label')}</label>
                        <select value={cWeather} onChange={e => setCWeather(e.target.value)} className="bg-bg-secondary border border-border-light rounded-xl p-3 text-xs font-black uppercase text-text-primary focus:border-gold outline-none">
                          <option value="">{t('sovereign_journal.weather_none')}</option>
                          {WEATHER.map(m => <option key={m} value={m}>{t(`sovereign_journal.weather_${m}`)}</option>)}
                        </select>
                      </div>
                    </div>

                    {/* Entry Type Selector */}
                    <div className="flex gap-2 bg-bg-secondary p-1.5 rounded-2xl border border-border-light">
                      {(['reflection', 'activity', 'milestone'] as const).map((type) => (
                        <button
                          key={type}
                          onClick={() => setCEntryType(type)}
                          className={`flex-1 py-3 px-4 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${cEntryType === type ? 'bg-gold text-black shadow-lg shadow-gold/20' : 'text-text-tertiary hover:text-text-primary'
                            }`}
                        >
                          {t(`sovereign_journal.${type}`)}
                        </button>
                      ))}
                    </div>
                    ...
                    <div className="flex items-center justify-between bg-bg-secondary border border-border-light p-4 rounded-xl mt-2">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h4 className="text-xs font-black uppercase tracking-widest">{t('sovereign_journal.vault')}</h4>
                          <p className="text-[10px] text-text-tertiary mt-0.5">{t('sovereign_journal.private_sub')}</p>
                        </div>
                      </div>
                      <button onClick={() => setCVisibility(cVisibility === 'vault' ? 'normal' : 'vault')} className={`w-12 h-6 rounded-full relative transition-colors ${cVisibility === 'vault' ? 'bg-gold' : 'bg-bg-tertiary border border-border-light'}`}>
                        <div className={`absolute top-1 bottom-1 w-4 rounded-full bg-white transition-all shadow-sm ${cVisibility === 'vault' ? 'right-1' : 'left-1'}`} />
                      </button>
                    </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              {Object.keys(monthGroups).length === 0 && (
                 <div className="col-span-4 text-center py-10 opacity-30">
                   <p className="text-[10px] font-black uppercase tracking-widest">{t('sovereign_journal.empty_msg')}</p>
                 </div>
              )}
            </div>
          </motion.div>
          ...
          >
            ...
            <div className="mb-10 text-center border-b border-border-light/50 pb-10">
              ...
              <h2 className="text-3xl md:text-4xl font-black text-text-primary tracking-tight mb-4">
                {activeEntry?.title || (activeEntry?.date ? new Date(String(activeEntry.date)).toLocaleDateString(lang === 'hi' ? 'hi-IN' : 'en-IN', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }) : '')}
              </h2>
            ...
            <div className="flex border-t border-border-light pt-8 justify-between items-center">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-border-light flex items-center justify-center text-xs font-black">
                 ...
                 <div>
                    <div className="text-[10px] font-black text-text-tertiary uppercase tracking-[0.2em]">{t('sovereign_journal.secure_entry')}</div>
                    <div className="text-sm font-bold text-text-primary">{t('sovereign_journal.vault_storage')}</div>
                 </div>
              </div>

              <button
                onClick={() => { if (activeEntry) { deleteEntry(activeEntry.id); setView('timeline'); } }}
