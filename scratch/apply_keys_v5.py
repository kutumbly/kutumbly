...
def apply_updates():
    ...
    for key, data in NEW_KEYS_DATA.items():
        # Build complete dictionary entry for all 12 languages
        langs_dict = {}
        for l in LANGUAGES:
            # Use provided translation, fallback to English if not available
            langs_dict[l] = data.get(l, data.get('en', 'Missing'))
    ...
    print(f"Successfully applied {len(NEW_KEYS_DATA)} type-safe updates to i18n.ts (v5)")
