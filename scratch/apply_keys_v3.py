...
def apply_updates():
    ...
    # Find the last entry before the closing brace of DICTIONARY
    dict_end_idx = content.rfind('};')
    if dict_end_idx == -1:
        print("Error: Could not find end of DICTIONARY object.")
        return

    # Find the last closing brace of a dictionary entry before the end
    last_entry_end = content.rfind('}', 0, dict_end_idx)
    if last_entry_end == -1:
    ...
    print(f"Successfully applied {len(NEW_KEYS)} updates to i18n.ts")
