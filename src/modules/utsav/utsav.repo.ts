...
export const utsavRepo = {
  ...
  getEvents: (db: Database | null, direction?: string): NevataEvent[] => {
    if (!db) return [];
  ...
  getUpcomingEvents: (db: Database | null, limit: number): NevataEvent[] => {
    if (!db) return [];
  ...
  createShagun: (db: Database | null, shagun: ShagunRecord) => {
    if (!db) return;
    db.run(
  ...
  updateFamilyLedger: (db: Database | null, familyName: string, amount: number, direction: 'given' | 'received') => {
    ...
    const ledger = runQuery<NevataLedgerEntry>(db, "SELECT * FROM nevata_family_ledger WHERE family_name = ?", [familyName]);
  ...
  getHistoricalDebt: (db: Database | null, familyName: string): { diya: number; mila: number } | null => {
    if (!db) return null;
    const res = runQuery<NevataLedgerEntry>(db, "SELECT diya, mila FROM nevata_family_ledger WHERE family_name = ? LIMIT 1", [familyName]);
